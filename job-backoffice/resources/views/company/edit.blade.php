        ...
        <div class="max-w-2xl p-6 mx-auto bg-white rounded-lg shadow-md ">
            <form action="{{ route('company.update', $company->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="industry" id="industry" class=" {{ $errors->has('industry') ? 'outline-red-500 outline outline-1' : '' }} block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('industry', $company->industry) }}" >
                    @error('industry')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
